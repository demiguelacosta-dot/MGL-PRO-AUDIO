...
if not "%REPO_URL%"=="" (
  ...
  git fetch origin
  git merge origin/main --allow-unrelated-histories -m "Integrate GitHub history" 2>nul
...
echo.
echo En Vercel importa el repositorio y agrega estas variables:
echo NEXT_PUBLIC_SUPABASE_URL
echo NEXT_PUBLIC_SUPABASE_ANON_KEY
if not "%YOUTUBE_API_KEY%"=="" echo NEXT_PUBLIC_YOUTUBE_API_KEY
if not "%YOUTUBE_API_KEY_2%"=="" echo YOUTUBE_API_KEY
echo.
echo Proyecto preparado. No cierres esta ventana hasta revisar Vercel.
pause
exit /b 0
...
echo Revisa si hay conflictos con archivos creados en ambos repositorios.
pause
exit /b 1
